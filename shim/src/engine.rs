...
mod tests {
    ...
    /// (what a LAN relay looks like to the node), `status_json` reports that
    /// relay as connected within a few seconds — the Overview's relay list
    /// end to end, minus the UI. The LAN entry is given twice with and
    /// without a trailing slash, and once more as the base relay, so the
    /// pool must hold exactly one copy. No public network: the base relay
    /// list is overridden and STUN points at a dead loopback port.
    #[test]
    fn status_reports_loopback_relay_connected() {
        ...
        let config = serde_json::json!({
            ...
            "enable_nostr": true,
            "nostr_relays": [relay],
            "extra_nostr_relays": [format!("{relay}/"), relay, "not a relay"],
